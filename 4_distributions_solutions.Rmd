...
1. Sort the points in the sample.

2. For the k-th value, compute $(k-.5)/N$ (maybe mention here that k is called the rank and that we actually use Rankit here to compute p but that there are other methods doing that?)and obtain the value where it should lie given a normal distribution. You can think of this as starting at $(k-.5)/N$ on the y-axis and going to the right until you hit the cumulative distribution function of a standard normal distribution. Then you go straight down until you hit the x-axis. The direct way to compute this number of is the inverse distribution function or quantile function, called *qnorm*. Find out how it works using ?. 
